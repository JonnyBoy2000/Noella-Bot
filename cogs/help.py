...
class Help():
	...
	async def help(self, ctx, *, inputs : str = None):
		...
		### Kawaii Help Inputs ###
			...
			embed = discord.Embed(title = "Love - Kawaii Command", description = f"Tell someone that you love her/him or {self.bot.user.name}.", colour = embed_color)
			embed.add_field(name = "Basic Usage: ", value = f"`{bot_prefix}love`", inline = False)
			embed.add_field(name = "Other Usage: ", value = f"`{bot_prefix}love [person]`", inline = False)
			embed.set_footer(text = "Do not use the example brackets when using the commands!")
			await ctx.send(embed = embed)

		elif inputs == "gg":
			embed = discord.Embed(title = "Good Game - Kawaii Command", description = f"Tell someone Good Game or {self.bot.user.name}.", colour = embed_color)
			embed.add_field(name = "Basic Usage: ", value = f"`{bot_prefix}gg`", inline = False)
			embed.add_field(name = "Other Usage: ", value = f"`{bot_prefix}gg [person]`", inline = False)
			embed.set_footer(text = "Do not use the example brackets when using the commands!")
			await ctx.send(embed = embed)
